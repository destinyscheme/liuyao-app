...
# 0. 網頁設定 & CSS
# ==============================================================================
st.set_page_config(page_title="六爻智能排盤-匯出強化版v22", layout="wide")
...
.hex-table { 
    ...
    font-size: 18px; 
    table-layout: fixed; 
    border: 2px solid #000; /* 保留外框 */
    margin-top: 10px;
}
.hex-table td { 
    padding: 8px 2px;
    border: none !important; /* 刪除所有內框線 */
    vertical-align: middle; 
    color: #000; 
}
/* 移除最後一列與最後一欄的邊框設定(因為已經全部none了，這裡只是保險) */
.hex-table tr:last-child td { border-bottom: none; }
.hex-table td:last-child { border-right: none; }

/* 無縫表格樣式 (雖然內框已除，但保留類別以防萬一需要微調) */
...
.header-row td { 
    ...
    font-weight: bold; 
    color: #000; 
    border-bottom: none !important; /* 標題列下方的線也移除 */
    padding-bottom: 10px;
...
<td class="small-text" style="font-size:0.85em;">{c_nayin_short}</td>
    ...
    # 準備資料 DataFrame
    export_data = []
    for line in lines_data[::-1]: # 調整順序為 初爻 -> 上爻
        row_dict = {
            "六神": line['god'],
            "藏伏": line['hidden'],
    ...
    if export_format == "試算表 (Excel/CSV)":
        ...
        <body>
            ...
            <p>{stars_row2_str}</p>
            <hr>
            {table_html.replace('width="100%"', 'width="100%" border="1"')}
        </body>
        </html>
        """
        st.download_button(
