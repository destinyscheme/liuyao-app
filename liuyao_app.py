...
def calculate_hexagram(numbers, day_stem, day_branch):
    ...
    # 【關鍵修正】：字典 TRIGRAMS 的 code 是 [Top, Mid, Bottom] (標準視覺)
    # 但 main_code 是 [Line1, Line2, Line3] (Bot, Mid, Top)
    # 所以在比對字典時，必須將 slice 反轉 [::-1]
    
    tri_map = {tuple(v["code"]): k for k, v in TRIGRAMS.items()}
    
    # 下卦 (Code: Bot->Top) -> Reverse to match Dict (Top->Bot)
    ...
    for i in range(6):
        ...
        c_rel = ELEMENT_RELATIONS.get((palace_element, c_el), "") # 變卦六親以主卦宮位為準
...
with st.sidebar:
    ...
    cols = st.columns(6)
    input_vals = []
    # 預設值改為產生 澤山咸 -> 地風升 的組合 (8,6,7,9,9,8)
    def_vals = [8, 6, 7, 9, 9, 8] 
    for i in range(6):
...
if btn or True:
    ...
    # 卦象表格 (完全對齊版)
    # 欄位分配：六神(10%) | 伏神(10%) | 納音(15%) | 主卦(35%) | 變(5%) | 變卦(25%)
